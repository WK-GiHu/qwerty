...
class Kiosk(tk.Tk):
    ...
    def on_grant_access(self, event):
        ...
        if event.state <10:
            self.details = FingerprintThread.result
            self.firstn = self.details[1]
            self.middlen = self.details[2]
            self.lastn = self.details[3]
            ...
            self.sq1 = self.details[10]
            self.ans = self.details[11]
            self.rf = self.details [12]
